    ...
    ''
      ...
      create_spaces() {
        ...
        if [[ $1 == "a" ]]; then
          ...
          set_wallpaper ${inputs.self.packages.${system}.wallpapers}/share/wallpapers/$(ls ${
            inputs.self.packages.${system}.wallpapers
          }/share/wallpapers/ | shuf -n 1)
          return 0
        fi
      ...
      set_wallpapers() {
        if [[ $(command -v yabai) ]]; then
          LOCAL_WALLPAPERS="$(realpath ${inputs.self.packages.${system}.wallpapers}/share/wallpapers/)"
