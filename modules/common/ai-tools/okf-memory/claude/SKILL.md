...
hooks:
  SessionStart:
    - hooks:
        - type: command
          command: "SH=\"${CLAUDE_SKILL_DIR}/scripts/inject-memory.sh\"; [ -f \"$SH\" ] && sh \"$SH\"; exit 0"
  UserPromptSubmit:
    - hooks:
        - type: command
          command: "SH=\"${CLAUDE_SKILL_DIR}/scripts/inject-memory.sh\"; [ -f \"$SH\" ] && sh \"$SH\"; exit 0"
...
session start and on every prompt — on Claude Code, via the hooks in this file's
frontmatter. On every other provider, read `.okf/MEMORY.local.md` and
`.okf/index.md` yourself before answering questions about prior decisions or
project history that aren't obvious from the code.
