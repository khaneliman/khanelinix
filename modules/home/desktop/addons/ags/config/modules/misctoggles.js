...
async function toggleSystemdService(serviceName, button) {
  ...
  exec(
    `pkexec bash -c "systemctl ${serviceState ? "disable" : "enable"
    } ${serviceName}"`,
  );
...
const ModuleRecord = (props = {}) =>
  Widget.Button({
    ...props,
    className:
      "button-minsize sidebar-button-nopad sidebar-button-alone-normal txt-small",
    onClicked: () => {
...
const SystemdService = (serviceName) => {
  ...
  return Widget.Button({
    className:
      "button-minsize sidebar-button sidebar-button-alone-normal txt-small",
    onClicked: (button) => {
