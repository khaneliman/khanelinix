      ...
      if (icon != "NO_ICON") {
        box.pack_start(
          Icon({
            ...
            valign: "center",
            setup: () =>
              box.toggleClassName("sidebar-notif-icon-material", true),
          }),
          false,
          true,
          0,
        );
      } else if (notifObject.urgency == "critical") {
        box.pack_start(
          MaterialIcon("release_alert", "hugeass", {
            hexpand: true,
            setup: () => {
      ...
      Box({
        ...
        children: [
          Label({
            ...
            justification: "right",
            setup: (label) => {
              const messageTime = GLib.DateTime.new_from_unix_local(
                notifObject.time,
              );
              if (
