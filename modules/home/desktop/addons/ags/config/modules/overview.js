...
function launchCustomCommand(command) {
  ...
  if (args[0] == ">raw") { // Mouse raw input
    ...
    execAsync([`bash`, `-c`, `${App.configDir}/scripts/switchwall.sh`, `&`])
      .catch(print);
...
const ExecuteCommandButton = ({ command, terminal = false }) =>
  searchItem({
...
const SearchButton = ({ text = "" }) =>
  searchItem({
    ...
    onActivate: () => {
      MenuService.close("overview");
      execAsync(["xdg-open", `https://www.google.com/search?q=${text}`]).catch(
        print,
      );
    },
  });
    ...
    setup: (menuItem) => {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        button.connect("activate", () => {
          execAsync([`${onClickBinary}`, `${thisWorkspace}`, `${i}`]).catch(
            print,
          );
        });
...
const client = (
  { address, size: [w, h], workspace: { id, name }, class: c, title },
) =>
  Widget.Button({
  ...
  const widget = Widget.Box({
    ...
    children: [Widget.EventBox({
      ...
      onPrimaryClickRelease: () => {
        execAsync([`bash`, `-c`, `hyprctl dispatch workspace ${index}`, `&`])
          .catch(print);
        MenuService.close("overview");
      },
...
const OverviewRow = (
  { startWorkspace = 1, workspaces = 5, windowName = "overview" },
) =>
  Widget.Box({
    ...
    onAccept: ({ text }) => { // This is when you press Enter
      ...
      if (startsWithNumber(text)) { // Eval on typing is dangerous, this is a workaround
        try {
          ...
          // copy
          execAsync(["bash", "-c", `wl-copy '${fullResult}'`, `&`]).catch(
            print,
          );
          MenuService.close("overview");
          return;
      ...
      if (
        !isAction && exec(`bash -c "command -v ${text.split(" ")[0]}"`) != ""
      ) {
        ...
        MenuService.close("overview");
        execAsync(["xdg-open", `https://www.google.com/search?q=${text}`])
          .catch(print);
      }
    },
    ...
    connections: [
      [App, (_b, name, visible) => {
        if (name == "overview" && !visible) {
          entryPromptRevealer.child.label = searchPromptTexts[
