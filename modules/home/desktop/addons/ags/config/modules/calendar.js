...
if (fileExists(`${App.configDir}/../../.cache/ags/user/todo.json`)) {
  todoJson = JSON.parse(
    Utils.readFile(`${App.configDir}/../../.cache/ags/user/todo.json`),
  );
} else {
  todoJson = [];
  Utils.writeFile(
    JSON.stringify(todoJson),
    `${App.configDir}/../../.cache/ags/user/todo.json`,
  )
    .catch((err) => print(err));
}
...
const CalendarDay = (day, today) =>
  Widget.Button({
    className: `sidebar-calendar-btn ${today == 1
...
const CalendarWidget = () => {
  const calendarMonthYear = Widget.Button({
    ...
    setup: (button) => {
      button.label = `${new Date().toLocaleString("default", { month: "long" })
  ...
  function shiftCalendarXMonths(x) {
    ...
    calendarMonthYear.label = `${monthshift == 0 ? "" : "• "}${newDate.toLocaleString("default", { month: "long" })
      } ${newDate.getFullYear()}`;
...
const drawTodoItems = (todoJson, isDone) =>
  todoJson.map((task, i) => {
...
const undoneItems = Widget.Scrollable({
  ...
  child: Widget.Box({
    vertical: true,
    children: drawTodoItems(todoJson, false),
  }),
});
const doneItems = Widget.Scrollable({
  child: Widget.Box({
    vertical: true,
    children: drawTodoItems(todoJson, true),
  }),
});
...
const StackButton = (parentBox, stackItemName, icon, name) =>
  Widget.Button({
    className:
