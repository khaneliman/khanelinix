...
in
{
  ...
  config = mkIf cfg.enable {
    home = {
      activation =
        mkIf (pkgs.stdenv.hostPlatform.isDarwin && !config.khanelinix.programs.terminal.tools.infat.enable)
          {
          ...
          ${config.khanelinix.user.name} = {
            ...
            settings = mkMerge [
              cfg.settings
              {
                ...
                "browser.download.dir" = lib.khanelinix.userDir config "download" "Downloads";
